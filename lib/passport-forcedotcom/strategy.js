...
Strategy.prototype.userProfile = function(token, done) {
  var self = this;
  var userid_split = self._oauthData.id.split('/');
  var userid = userid_split[userid_split.length-1];
  
  //Set useAuthorizationHeaderforGET to true so that we can make userInfo call to get actual user info.
  this._oauth2.useAuthorizationHeaderforGET(true);

  this._oauth2.get(self._oauthData.instance_url+ '/services/data/v20.0/sobjects/User/' +userid, token, function(err, body, res) {
    if (err) {
      return done(err);
    }
    try {
